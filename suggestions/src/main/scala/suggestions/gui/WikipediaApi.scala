...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs.materialize.filter {!_.isInstanceOf[OnCompleted[T]]} map{
     ...
     */
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = (for (t <- obs) yield {
